...
try:
    ...
    try:
        ...
        print( flush=True,value="Note: moviepy.video.fx.resize not found (may be integrated in newer MoviePy).")
...
try:
    if moviepy_resize: # Check if the module was imported
        ...
        print( flush=True,value="Applied patched resizer.")
    else:
         print( flush=True,value="Skipping resizer patch: moviepy.video.fx.resize not found.")
except Exception as e:
    print( flush=True,value=f"Could not apply patched resizer: {e}")
...
def blur_subtitles_in_video_unified(
    ...
    def _determine_subtitle_bbox_from_frame(
        ...
        try:
            ...
            print( flush=True,value=f"An error occurred during OCR: {e}")
            return None
        ...
        if not subtitle_word_boxes:
            print( flush=True,value="No subtitle-like text found in the expected region or with sufficient confidence on the sample frame.")
            return None
        ...
        print( flush=True,value=f"Determined subtitle bounding box (x1,y1,x2,y2): {final_bbox}")
        return final_bbox
    ...
    if not os.path.exists(video_path):
        print( flush=True,value=f"Error: Input video '{video_path}' not found.")
        return

    # if tesseract_cmd_path:
    #     pytesseract.pytesseract.tesseract_cmd = tesseract_cmd_path
    #     print( flush=True,value=f"Using Tesseract from: {tesseract_cmd_path}")
    ...
    #     print( flush=True,value="Warning: Tesseract command path not specified and tesseract.exe might not be in PATH.")
    #     print( flush=True,value="If OCR fails, please provide 'tesseract_cmd_path'.")


    print( flush=True,value=f"Loading video: {video_path}")
    clip = None
    processed_clip = None
    try:
        clip = VideoFileClip(video_path)
    except Exception as e:
        print( flush=True,value=f"Error loading video '{video_path}': {e}")
        return

    print( flush=True,value=f"Extracting sample frame at {sample_time_sec} seconds...")
    try:
        ...
        if debug_save_frames:
            ...
            print( flush=True,value="Saved 'debug_sample_frame.png'")
    except Exception as e:
        print( flush=True,value=f"Error extracting sample frame: {e}")
        if clip: clip.close()
        return

    print( flush=True,value="Determining subtitle bounding box using Pytesseract...")
    ...
    if subtitle_bbox is None:
        print( flush=True,value="Could not determine subtitle bounding box. Aborting video processing.")
        if clip: clip.close()
        return
    ...
    if debug_save_frames:
        ...
        print( flush=True,value="Saved 'debug_sample_frame_with_bbox.png' for verification.")
    ...
    print( flush=True,value="Applying blur to video frames...")
    try:
        ...
        if output_dir and not os.path.exists(output_dir):
            ...
            print( flush=True,value=f"Created output directory: {output_dir}")

        processed_clip.write_videofile(output_path, codec="libx264", audio_codec="aac")
        print( flush=True,value=f"Successfully processed video saved to: {output_path}")
    except Exception as e:
        print( flush=True,value=f"Error during video processing or writing to '{output_path}': {e}")
        print( flush=True,value="Ensure FFmpeg is installed and accessible by MoviePy. Check write permissions.")
    finally:
        if clip: clip.close()
...
# def search_tiktok_links_google(api_keys, cx_id, query, num_results=20, max_retries=3):
    ...
    #             except Exception as e:
    #                 print( flush=True,value=f"[Retry {tries+1}] Error: {e}")
    #                 tries += 1
...
def get_yt_dlp_info(video_url):
    ...
    if COOKIE_FILE_PATH and os.path.exists(COOKIE_FILE_PATH):
        ...
        print( flush=True,value=f"yt-dlp: Using cookie file: {COOKIE_FILE_PATH}")
    elif COOKIE_FILE_PATH:
        print( flush=True,value=f"yt-dlp: Cookie file path provided but not found: {COOKIE_FILE_PATH}")
    ...
    try:
        with yt_dlp.YoutubeDL(YDL_OPTS) as ydl:
            ...
            if not direct_url and 'formats' in info:
                print( flush=True,value=f"yt-dlp: Top-level URL missing for {video_url}. Checking formats list...")
                ...
                if selected_format:
                    print( flush=True,value=f"yt-dlp: Found URL in formats list (Format ID: {selected_format.get('format_id')})")
            ...
            if direct_url:
                ...
                print( flush=True,value=f"Warning: Could not extract direct URL for {video_url} even after checking formats. Info keys: {info.keys()}")
...
def download_vid_ytdlp(video_url, output_dir="downloads", filename_template="%(title).80s.%(ext)s"):
    ...
    try:
        ...
        print( flush=True,value=f"[yt-dlp error] {e}")
        return None
    except Exception as e:
        print( flush=True,value=f"[unexpected error] {e}")
        return None
...
def chatGPT(prompt, client, model="gpt-4o", temperature=1.0):
    ...
    try:
        ...
        # Consider logging the full error: print( flush=True,value=f"OpenAI Error: {e}")
        return None
...
def claude(prompt , model = "claude-3-7-sonnet-20250219", temperature=1 , is_thinking = False, max_retries = 10):
    ...
    while tries < max_retries:
        try:
            ...
            print( flush=True,value=message)
            return message.content[0].text
...
def generate_audio_with_timestamps(text, client, voice_id="sage"):
    ...
    try:
        ...
        try:
            ...
            # print( flush=True,value=f"Audio volume boosted for {temp_audio_path}") # Debug log
    ...
    if temp_audio_path and os.path.exists(temp_audio_path):
        try:
            ...
            print( flush=True,value=f"Cleaned up temp audio file on error: {temp_audio_path}") # Debug log
    ...
    for i in range(0, len(word_timings), words_per_group):
        ...
        if group_words_data:
            ...
            if 'start' in group_words_data[0] and 'end' in group_words_data[-1]:
                 ...
                 #    print( flush=True,value=f"Skipping invalid subtitle group: {group_words_data}")
            # else: # Optional: Log missing start/end
            #    print( flush=True,value=f"Skipping group due to missing time data: {group_words_data}")
...
def download_with_ytdlp(video_url, cookie_file_path=None):
    ...
    try:
        ...
        ydl_opts = {
            ...
            # 'progress_hooks': [lambda d: print( flush=True,value=d['status'], d.get('filename'))], # Example hook
        }
...
def download_direct_url(url, suffix=".mp4"):
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as temp_file:
            ...
            print( flush=True,value=f"Attempting to download direct URL '{url}' to temp file: {local_path}")
        ...
        print( flush=True,value=f"✔️ Direct download successful: {local_path}")
        # Basic check
        if os.path.getsize(local_path) == 0:
            print( flush=True,value=f"⚠️ Warning: Direct download resulted in an empty file: {local_path}")
        ...
        print( flush=True,value=f"❌ Direct Download failed (Network/HTTP Error): {e}")
    except Exception as e:
        print( flush=True,value=f"❌ Direct Download failed (Other Error): {e}")
...
def sync_search_data():
    ...
    try:
        ...
        # print( flush=True,value="Data editor sync complete.") # Debug log
            ...
            if platform =='tk':
                ...
                with st.spinner(f"Fetching video details for '{title}' (Job: {fetch_job_key} {standard_url})..."):
                    ...
                    if standard_url and standard_url in st.session_state.get('resolved_vid_urls', {}):
                        ...
                        print( flush=True,value=f"Cache hit for {standard_url}") # Debug
                    elif standard_url:
                        print( flush=True,value=f"Cache miss - Fetching NEW URL info for {standard_url}") # Debug
